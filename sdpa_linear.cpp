...
_Float128 Lal::getMinEigen(DenseMatrix& lMat,
		       DenseMatrix& xMat,
		       DenseMatrix& Q,
		       ...
		       Vector& workVec)
{
  _Float128 alpha,beta,value;
  _Float128 min = 1.0e+51, min_old = 1.0e+52, min_min= 1.0e+50;
  _Float128 error = 1.0e+10;
  ...
  q.setZero();
  r.initialize(MONE);
  beta = sqrtq((_Float128)nDim);  // norm of "r"

  // nakata 2004/12/12
  while (k<nDim && k<sqrtq((_Float128)nDim)+10
	 && beta > 1.0e-16
    ...
    if ( kk>=sqrtq((_Float128)k) || k==nDim-1 || k>sqrtq((_Float128)nDim+9) ) {
      kk = 0;
...
_Float128 Lal::getMinEigenValue(DenseMatrix& aMat,
			     Vector& eigenVec,
			     Vector& workVec)
{
  // aMat is rewritten.
...
bool Lal::getInnerProduct(_Float128& ret, Vector& aVec, Vector& bVec)
{
  int N = aVec.nDim;
  if (N != bVec.nDim) {
...
bool Lal::getInnerProduct(_Float128& ret,
			  ...
			  SparseMatrix& aMat, DenseMatrix& bMat)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index=0; index<aMat.NonZeroCount; ++index) {
      ...
      _Float128 value = aMat.sp_ele      [index];
      // rMessage("i=" << i << "  j=" << j);
      if (i==j) {
    ...
    for (int index=amari,counter = 0;
	 counter < shou ; ++counter, index+=4) {
      ...
      _Float128 value1 = aMat.sp_ele      [index];
      _Float128 ret1;
      // rMessage("i=" << i << "  j=" << j);
      if (i1==j1) {
	...
	ret1= value1*(bMat.de_ele[i1+bMat.nRow*j1]
      ...
      _Float128 value2 = aMat.sp_ele      [index+1];
      _Float128 ret2;
      // rMessage("i=" << i << "  j=" << j);
      if (i2==j2) {
	...
	ret2= value2*(bMat.de_ele[i2+bMat.nRow*j2]
      ...
      _Float128 value3 = aMat.sp_ele      [index+2];
      _Float128 ret3;
      // rMessage("i=" << i << "  j=" << j);
      if (i3==j3) {
	...
	ret3= value3*(bMat.de_ele[i3+bMat.nRow*j3]
      ...
      _Float128 value4 = aMat.sp_ele      [index+3];
      _Float128 ret4;
      // rMessage("i=" << i << "  j=" << j);
      if (i4==j4) {
...
int Lal::rdpotf2_(char*uplo, int *n, double *a, int *lda, int *info)
{
  ...
  for (int j = 0; j <*n; ++j) {
    ...
    if (j < *n-1) {
      int i = *n-1 - j;
      dgemv_f77("No transpose", &i, &j, &DMONE, &a[j + 1],
	     lda, &a[j], lda, &DONE,
  ...
  *info = 0;
  ...
  if (nb <= 1 || nb >= *n) {
    ...
    for (int j = 0; j < *n; j += nb) {
      int jb = min(nb,*n- j);
      dsyrk_f77("Lower", "No transpose", &jb,
	     &j, &DMONE, &a[j], lda,
	     &DONE, &a[j+nRow*j], lda, strlen("Lower"), strlen("No transpose"));
      // Here is point.
	...
	*info = *info + j - 1;
	return 0;
      ...
      if (j + jb <= *n-1) {
	int i = *n - j - jb;
	dgemm_f77("No transpose", "Transpose", &i, &jb,
	       &j, &DMONE, &a[j + jb], lda, &a[j], lda, 
	       &DONE, &a[(j + jb)+nRow*j], lda, strlen("No transpose"), strlen("Transpose"));
	dtrsm_f77("Right", "Lower", "Transpose", "Non-unit",
	       &i, &jb, &DONE, &a[j+nRow*j], lda,
	       &a[(j + jb)+nRow*j], lda, strlen("Right"), strlen("Lower"), strlen("Transpose"), strlen("Non-unit"));
      }
    }
  }
#endif
...
bool Lal::choleskyFactorWithAdjust(DenseMatrix& aMat)
{
  ...
  return _SUCCESS;
#if 0
  _Float128 ZERO_DETECT = 1.0e-3;
  _Float128 NONZERO = 1.0e-7;
  // no idea version
  ...
  while (start<aMat.nRow) {
    ...
    _Float128 wrong = aMat.de_ele[start+start*aMat.nRow];
	...
	_Float128& migi  = aMat.de_ele[start+(start+j)*aMat.nRow];
	_Float128& shita = aMat.de_ele[(start+j)+start*aMat.nRow];
	_Float128& mishi = aMat.de_ele[(start+j)+(start+j)*aMat.nRow];
...
#define TUNEUP 0
  ...
  case SparseMatrix::DENSE:
    xVec.copyFrom(bVec);
    F77_FUNC (dtrsv, DTRSV)("Lower", "NoTranspose", "NonUnit",
	   &aMat.nRow, aMat.de_ele, &aMat.nCol, xVec.ele,&IONE);
    F77_FUNC (dtrsv, DTRSV)("Lower", "Transpose", "NonUnit",
	   &aMat.nRow, aMat.de_ele, &aMat.nCol, xVec.ele,&IONE);
  return _SUCCESS;
  }
#endif
		   ...
		   _Float128* scalar)
{
  ...
  case SparseMatrix::SPARSE:
	...
	int        i = aMat.row_index   [index];
	int        j = aMat.column_index[index];
	_Float128 value = aMat.sp_ele      [index] * (*scalar);
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
	    retMat.de_ele[i+retMat.nRow*t] +=
		   ...
		   _Float128* scalar)
{
  ...
  case SparseMatrix::SPARSE:
	...
	_Float128 value = bMat.sp_ele      [index];
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
	  ...
	  #else
	  Raxpy(bMat.nCol,value,&aMat.de_ele[aMat.nRow*j],1,
		 &retMat.de_ele[retMat.nRow*j],1);
	  #endif
	}
      } // end of 'for index'
    } else { // scalar!=NULL
	...
	int        i = bMat.row_index   [index];
	int        j = bMat.column_index[index];
	_Float128 value = bMat.sp_ele      [index] * (*scalar);
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
  ...
  case SparseMatrix::DENSE:
    ...
    Rgemm("NoTranspose","NoTranspose",retMat.nRow,retMat.nCol,aMat.nCol,
...
bool Lal::multiply(DenseMatrix& retMat,
		   DenseMatrix& aMat, _Float128* scalar)
{
		   ...
		   _Float128* scalar)
{
  ...
  case DenseMatrix::DENSE:
    ...
    Rgemv("NoTranspose",aMat.nRow,aMat.nCol,
	   *scalar,aMat.de_ele,aMat.nRow,bVec.ele,1,
	   0.0,retVec.ele,1);
    break;
  case DenseMatrix::COMPLETION:
    rError("no support for COMPLETION");
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index=amari,counter=0;
	 counter<shou; ++counter,index+=4) {
      ...
      _Float128 value1 = bMat.sp_ele      [index] * (*scalar);
      if (i1!=j1) {
	retMat.de_ele[i1+retMat.nCol*j1] += value1;
	retMat.de_ele[j1+retMat.nCol*i1] += value1;
      } else {
	retMat.de_ele[i1+retMat.nCol*i1] += value1;
      ...
      _Float128 value2 = bMat.sp_ele      [index+1] * (*scalar);
      if (i2!=j2) {
	retMat.de_ele[i2+retMat.nCol*j2] += value2;
	retMat.de_ele[j2+retMat.nCol*i2] += value2;
      } else {
	retMat.de_ele[i2+retMat.nCol*i2] += value2;
      ...
      _Float128 value3 = bMat.sp_ele      [index+2] * (*scalar);
      if (i3!=j3) {
	retMat.de_ele[i3+retMat.nCol*j3] += value3;
	retMat.de_ele[j3+retMat.nCol*i3] += value3;
      } else {
	retMat.de_ele[i3+retMat.nCol*i3] += value3;
      ...
      _Float128 value4 = bMat.sp_ele      [index+3] * (*scalar);
      if (i4!=j4) {
	retMat.de_ele[i4+retMat.nCol*j4] += value4;
	retMat.de_ele[j4+retMat.nCol*i4] += value4;
      } else {
...
bool Lal::plus(BlockVector& retVec,
	       BlockVector& aVec,
	       BlockVector& bVec, _Float128* scalar)
{
...
bool Lal::let(DenseMatrix& retMat, const char eq,
	      ...
	      DenseMatrix& bMat, _Float128* scalar)
{
  _Float128 minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(DenseMatrix& retMat, const char eq,
	      DenseMatrix& aMat, const char op,
	      SparseMatrix& bMat, _Float128* scalar)
{
  _Float128 minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(_Float128& ret, const char eq,
	      Vector& aVec, const char op,
	      Vector& bVec)
{
  switch (op) {
...
bool Lal::let(DenseLinearSpace& retMat, const char eq,
	      DenseLinearSpace& aMat, const char op,
	      DenseLinearSpace& bMat, _Float128* scalar)
{
  _Float128 minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(DenseLinearSpace& retMat, const char eq,
	      DenseLinearSpace& aMat, const char op,
	      SparseLinearSpace& bMat, _Float128* scalar)
{
  _Float128 minus_scalar;
  switch (op) {
  case '+':
