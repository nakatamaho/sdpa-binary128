...
void Newton::computeFormula_SDP(InputData& inputData,
				_Float128 DenseRatio, _Float128 Kappa)
{
  ...
  for (int l=0; l<SDP_nBlock; ++l) {
    ...
    for (int k=0; k < inputData.SDP_nConstraint[l]; k++){
      ...
      _Float128 inz = inputData.A[i].SDP_sp_block[ib].NonZeroEffect;

      _Float128 f1,f2,f3;
      _Float128 n       = inputData.A[i].SDP_sp_block[ib].nRow;
      _Float128 up      = upNonZeroCount[i*SDP_nBlock + l];
			  ...
			  WorkVariables& work)
{
  ...
  _Float128 target = beta.value*mu.current;
...
void Newton::calF1(_Float128& ret, DenseMatrix& G,
		    SparseMatrix& Aj)
{
  Lal::let(ret,'=',Aj,'.',G);
}

void Newton::calF2(_Float128& ret,
		    DenseMatrix& F, DenseMatrix& G,
		    DenseMatrix& X, SparseMatrix& Aj,
		    bool& hasF2Gcal)
{
  int alpha,beta;
  _Float128 value1,value2;
...
void Newton::calF3(_Float128& ret,
		    DenseMatrix& F, DenseMatrix& G,
		    ...
		    SparseMatrix& Ai, SparseMatrix& Aj)
{
  ...
  for (int index1=0; index1<Aj.NonZeroCount; ++index1) {
    ...
    _Float128 value1 = Aj.sp_ele[index1];
    sum = 0.0;
    for (int index2=0; index2<Ai.NonZeroCount; ++index2) {
      int gamma = Ai.row_index[index2];
      int delta  = Ai.column_index[index2];
      _Float128 value2 = Ai.sp_ele[index2];
      _Float128 plu = value2*invZ.de_ele[delta+invZ.nCol*beta]
        * X.de_ele[alpha+X.nCol*gamma];
      sum += plu;
      if (gamma!=delta) {
        _Float128 plu2 = value2*invZ.de_ele[gamma+invZ.nCol*beta]
    ...
    for (int index2=0; index2<Ai.NonZeroCount; ++index2) {
      ...
      int delta  = Ai.column_index[index2];
      _Float128 value2 = Ai.sp_ele[index2];
      _Float128 plu = value2*invZ.de_ele[delta+invZ.nCol*alpha]
        * X.de_ele[beta+X.nCol*gamma];
      sum += plu;
      if (gamma!=delta) {
        _Float128 plu2 = value2*invZ.de_ele[gamma+invZ.nCol*alpha]
...
#if 0
    TimeEnd(B_NDIAG_END1);
    _Float128 t = TimeCal(B_NDIAG_START1,B_NDIAG_END1);
    switch (formula) {
    case F1: com.B_F1 += t; break;
    case F2: com.B_F2 += t; break;
    case F3: com.B_F3 += t; break;
				   ...
				   ComputeTime& com)
{
  ...
  for (int l=0; l<LP_nBlock; ++l) {
    _Float128 xMat = currentPt.xMat.LP_block[l];
    _Float128 invzMat = currentPt.invzMat.LP_block[l];

      for (int k1=0; k1<inputData.LP_nConstraint[l]; k1++) {
	int i = inputData.LP_constraint[l][k1];
	int ib = inputData.LP_blockIndex[l][k1];
	//	int inz = inputData.A[i].LP_sp_block[ib].NonZeroEffect;
	_Float128 Ai = inputData.A[i].LP_sp_block[ib];

	for (int k2=k1; k2<inputData.LP_nConstraint[l]; k2++) {
	  int j = inputData.LP_constraint[l][k2];
	  int jb = inputData.LP_blockIndex[l][k2];
	  //	  int jnz = inputData.A[j].LP_sp_block[jb].NonZeroEffect;
	  _Float128 Aj = inputData.A[j].LP_sp_block[jb];
...
void Newton::display_sparse_bMat(FILE* fpout)
{
  ...
  for (int index=0; index<sparse_bMat.NonZeroCount; ++index) {
    ...
    _Float128 value = sparse_bMat.sp_ele[index];
    int ii = ordering[i];
    int jj = ordering[j];
